...
<div id="right">
	...
	<!-----------titile------------> 
	<div class="right_top">
    	<div class="top_title bill_tit">我要反馈</div>
    </div>
    <!-----------titile end------------> 
    <!-----------box------------>
    <div id="lostlendbids">
     ...
     <form name="form1" method="post"  action="?" onSubmit="return CheckForm();">
									<fieldset>
										...
										<div class="control-group">											
											...
											<div class="controls">
												<input type="text"  name="title" type="text" id="title" > <font color="#FF0000">*</font>
											</div> <!-- /controls -->				
										</div> <!-- /control-group -->
										
										
										<div class="control-group">											
											<label class="control-label" for="mailcontent">内  容：</label>
											<div class="controls">
												<textarea  id="mailcontent" name="mailcontent"  rows="10" cols="80" ></textarea>
											</div> <!-- /controls -->				
										</div> <!-- /control-group -->
										...
										<div class="form-actions">
											<input name="submit" id="submit" type="submit" class="btn" value="发  送" onclick="javascript:return confirm('您确认要反馈吗？');"> 
...
<table width="100%" cellpadding="10" cellspacing="1" border="0" align="center" >
      <?php
	  	$pagesize = 5; //设置每页记录数 
	  	$sql = "SELECT * FROM `mail` where uid=".$us['id']."";
		...
		$start = $pagesize * ($p - 1); //计算起始记录 
      	$sql = "SELECT * FROM `mail` WHERE uid=".$us['id']." order by fdate desc limit ".$start.",".$pagesize;
		if($query = mysql_query($sql)){
		while ($row=mysql_fetch_array($query)){
	  ?>
      	<tr>
        <td width="70%" align="left"><a href="mailcontent.php?nid=<?=$row['id']?>" style="text-decoration:none"><?=$row['title']?></a></td>
        <td align="center"><?php if ($row['isread']==0){?><span style="color:red; ">未回复</span><?php }elseif($row['isread']==1){?>已回复<?php }?></td>
