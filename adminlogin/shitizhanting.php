...
// }
	...
	if($_POST['id']){
		...
		// $member['yarea2']=0;
		// if($sys['zhanting']==0){
		// 	edit_sql("update `systemparameters` set zhanting=1,date='".$ti."' where id=1");
		// }
		edit_update_cl('member',$member,$us['id']);
		
            action::record("审核实体展厅", $us['id'], $_SESSION['adminid'], 0);          
            alert('确认成功.','?');
...
<?php
      ...
      <tr>
        ...
        <td style="background:<?=$_color?>" align="center"><?=$row['date1']?></td>
        <td style="background:<?=$_color?>" align="center"><?=$row['date2']?></td>
