...
<!--        <td align="center">操作人</td>-->
		...
		<?php
	  	$pagesize = 20; //设置每页记录数 
	  	$sql = "SELECT id FROM `ulevelup` WHERE issh=1 ".$_SESSION['Search']." ".$_SESSION['SearchTime']."";
		...
		$start = $pagesize * ($p - 1); //计算起始记录 
      	$sql = "SELECT * FROM `ulevelup` WHERE issh=1 ".$_SESSION['Search']." ".$_SESSION['SearchTime']." order by udate desc limit ".$start.",".$pagesize;
		if($query = mysql_query($sql)){
...
<!--        <td align="center"><?=$xx?></td>-->
        ...
        <?php if ($sid){?>
        <td align="center"><?=$sid['loginname']?></td>
        <?php }else{?>
<!--        <td align="center">自主升级</td>-->
        <?php }?>
