...
$oid=$_GET['oid'];
if($_GET['o']){
    $oo= getOne("select id from orders where ordersnumber='{$_GET['o']}'");
    $oid=$oo['id'];
}
$page=$_GET['page'];
$lx=$_GET['lx'];
...
if ($orders==null){
	echo "<script language=javascript>alert('订单不存在！');window.location.href='chakanshuju.php'</script>";
}
...
if ($_POST['goback']){
	echo "<script language=javascript>window.location.href='chakanshuju.php?page=".$page."'</script>";
}
?>
...
<!--          物流公司：
      ...
      <tr>
        ...
        foreach ($arr as $key => $row) {
            $goods=getOne("select id,goodsimg from goods where id={$row['goodid']}");
            if($goods['goodsimg']){
      ...
      <tr>
        ...
        <td align="center"><img src="../upload/<?=$img?>" width=100 height=100 border=0></td>
