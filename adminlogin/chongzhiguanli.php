...
$cheuid_arr = $_POST['UID'];
	foreach ((array)$cheuid_arr as $id)
	{
		...
		if ($chongzhi['isgrant']==0){
			...
			}elseif($chongzhi['lx']==2){
				$us_update['cfxf']=$us['cfxf']+$chongzhi['jine'];	
			}
			action::record("充值确认", $chongzhi['uid'], $_SESSION['adminid'],$chongzhi['jine']);
...
<div class="mainBox">
	  ...
	  ?>
        ...
        <td align="center"><?php if ($row['lx']==1){?>A积分<?php }elseif($row['lx']==2){?>B积分<?php }?></td>
        
         <!-- <td align="center"><?php if ($row['hklx']==1){?>微信<?php }elseif($row['hklx']==2){?>支付宝<?php }elseif($row['hklx']==3){?>银行卡<?php }?></td>  -->
		<td align="center"><input type="button" class="button" id="button3" name="button3" value="查看" onClick="window.location.href='spxqs.php?id=<?=$row['id']?>'" /></td>
		<td align="center"><img src="../upload/<?=$row['goodsimg']?>" width=80 height=80 border=0></td>
	  ...
	  ?>
                        	<th colspan="9">
                            <input type="submit"  name="button" id="button" value="充值确认" onClick="{if(confirm('您确定要进行充值确认吗?')){this.document.selform.submit();return true;}return false;}"/>
