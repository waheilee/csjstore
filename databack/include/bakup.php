...
if($a_type=='bakout'){
	if(empty($action)){
		...
		if($stop==1){
			...
			if($step>1){
				for($i=1;$i<=$f_num;$i++){
					$bakfile.='<a href="data/'.$pre.$i.'.rar">'.$pre.$i.'.rar</a><br>';
				}
			}
			adminmsg('bakup_out');
		}
	...
	if(empty($action)){
		...
		while($file = readdir($handle)){
			if((!$dbtablepre || eregi("^pw_",$file) || eregi("^$dbtablepre",$file)) && eregi("\.rar$",$file)){
		...
		if(!$count){
			...
			$handle=opendir(D_P.'data');
			while($file = readdir($handle)){
				if(eregi("^$pre",$file) && eregi("\.rar$",$file)){
					$count++;
				}
			}
		}
		!$step && $step=1;
		/*
		$sql=readover(D_P.'data/'.$pre.$step.'.rar');
		bakindata($sql);
		*/
		bakindata(D_P.'data/'.$pre.$step.'.rar');
		$i=$step;
		$step++;
